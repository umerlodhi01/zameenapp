      ...
      <template v-slot:default>
        ...
        <tbody v-if="ads">
          <tr v-for="ad in ads" :key="ad._id">
            ...
            <td>
              <v-tooltip left v-if="ad.status === 1">
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    right
                    v-on="on"
                    v-bind="attrs"
                    color="error"
                    @click.prevent="disableAd(ad._id)"
                  >
                    mdi-book-cancel
                  </v-icon>
                </template>
                <span>Disable AD</span>
              </v-tooltip>
              <v-tooltip left v-if="ad.status === 2">
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    right
                    v-on="on"
...
export default {
  ...
  methods: {
    ...
    deleteAD(id) {
      this.$store.dispatch("ads/deleteAd", id).then((response) => {
        if (response.data.success) {
          this.snackbar.snackbar = true;
          this.snackbar.text = "AD Deleted Successfully";
          this.snackbar.color = "error";
          this.countTotalAds();
          this.getADs(this.page);
        }
    ...
    disableAd(id) {
      this.$store.dispatch("ads/disableAd", id).then((response) => {
        if (response.data.success) {
          this.snackbar.snackbar = true;
          this.snackbar.text = "AD Disabled Successfully";
          this.snackbar.color = "error";
          this.countTotalAds();
          this.getADs(this.page);
        }
