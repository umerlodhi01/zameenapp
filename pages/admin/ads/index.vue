<template>
  <div class="pa-5">
    <v-btn @click="dialog = !dialog" color="success" class="ma-2 white--text">
      Add Property Ads
      ...
      <template v-slot:default>
        ...
        <tbody v-if="ads">
          <tr v-for="ad in ads" :key="ad._id">
            ...
            <td>{{ ad.commision }}</td>
            <td>
              <v-btn color="success" @click.prevent="viewAD(ad._id)"
                >View AD
                <v-icon right dark> mdi-eye-arrow-left-outline </v-icon></v-btn
              >
            </td>
            <td>
              <v-icon color="error" @click.prevent="deleteAD(ad._id)">
                mdi-delete
              </v-icon>
