<template>
  <div class="pa-5">
    ...
    >
      <v-card elevation="2" class="pa-3" v-if="data">
        ...
        <tbody v-if="purchases">
          <tr v-for="purchase in purchases" :key="purchase._id">
            ...
            <td>
              ...
              <v-tooltip left v-if="purchase.status === 3">
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    :disabled="purchase.paymentId.length < 1"
                    right
                    ...
                    v-bind="attrs"
                    color="orange"
                    @click.prevent="unVerifyPayment(purchase._id)"
                  >
                    mdi-cash-remove
                  </v-icon>
                </template>
                <span>Unverify Payment</span>
              </v-tooltip>
              <v-tooltip left>
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    :disabled="purchase.status > 1"
                    v-on="on"
                    v-bind="attrs"
                    color="error"
                    @click.prevent="deleteOrderConfirm(purchase._id)"
                  >
                    mdi-delete
                  </v-icon>
                </template>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      countTotalPurchases: "purchases/countTotalPurchases",
    }),
    viewOrder(id) {
      this.$store
    ...
    deleteOrder() {
      this.$store
        .dispatch("purchases/deleteOrder", this.deleteId)
        .then((response) => {
          if (response.data.success) {
            this.snackbar.snackbar = true;
            this.snackbar.text = "Order Deleted Successfully";
            this.snackbar.color = "error";
            this.deleteDialog = false;
            this.getPurchases(this.page);
    ...
    unVerifyPayment(id) {
      this.$store.dispatch("purchases/unVerifyPayment", id).then((response) => {
        if (response.data.success) {
          this.snackbar.snackbar = true;
          this.snackbar.text = "Payment Unverified Successfully";
          this.snackbar.color = "error";
          this.deleteDialog = false;
          this.getPurchases(this.page);
          this.countTotalPurchases();
    ...
    viewPayment(id) {
      this.$store.dispatch("purchases/paymentDetails", id).then(() => {
